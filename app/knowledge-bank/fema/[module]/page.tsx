...
import { Globe, ArrowLeft, ArrowRight, ThumbsUp, ThumbsDown, MessageCircle } from "lucide-react";
...
export async function generateStaticParams() {
  ...
  return articles.map((a) => ({ module: a.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { module: mod } = await params;
  const article = getKnowledgeArticle("fema", mod);
...
export default async function FEMAModulePage({ params }: PageProps) {
  const { module: mod } = await params;
  const article = getKnowledgeArticle("fema", mod);
  if (!article) notFound();

  const articles = getKnowledgeArticles("fema");
  const currentIndex = articles.findIndex((a) => a.slug === mod);
    ...
    <div className="mx-auto max-w-7xl px-4 py-10 lg:grid lg:grid-cols-[260px_1fr] lg:gap-10">
      ...
      <aside className="hidden lg:block">
        <div className="sticky top-24">
          <div className="flex items-center gap-2 mb-4">
            <Globe className="h-4 w-4 text-fema-purple" />
            <span className="text-xs font-medium text-fema-purple uppercase tracking-wider">
              FEMA
            </span>
          </div>
          <nav className="space-y-1">
            {articles.map((a) => (
              <Link
                key={a.slug}
                href={`/knowledge-bank/fema/${a.slug}`}
      ...
      <article className="min-w-0">
        <div className="flex items-center gap-2 text-xs text-muted mb-6">
          ...
          <span>/</span>
          <Link href="/knowledge-bank/fema" className="hover:text-gold transition-colors">
            FEMA
          </Link>
          <span>/</span>
          <span className="text-text">Module {article.frontmatter.chapter}</span>
        </div>
        ...
        <div className="mt-12 grid gap-4 sm:grid-cols-2 border-t border-white/[0.06] pt-8">
          {prev ? (
            <Link
              href={`/knowledge-bank/fema/${prev.slug}`}
          ...
          {next && (
            <Link
              href={`/knowledge-bank/fema/${next.slug}`}
