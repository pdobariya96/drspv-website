...
import { Receipt, ArrowLeft, ArrowRight, ThumbsUp, ThumbsDown, MessageCircle } from "lucide-react";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const article = getKnowledgeArticle("gst", mod);
    ...
    <div className="mx-auto max-w-7xl px-4 py-10 lg:grid lg:grid-cols-[260px_1fr] lg:gap-10">
      ...
      <aside className="hidden lg:block">
        <div className="sticky top-24">
          <div className="flex items-center gap-2 mb-4">
            <Receipt className="h-4 w-4 text-gst-green" />
            <span className="text-xs font-medium text-gst-green uppercase tracking-wider">
              GST
            </span>
          </div>
          <nav className="space-y-1">
            {articles.map((a) => (
              <Link
                key={a.slug}
                href={`/knowledge-bank/gst/${a.slug}`}
      ...
      <article className="min-w-0">
        <div className="flex items-center gap-2 text-xs text-muted mb-6">
          ...
          <span>/</span>
          <Link href="/knowledge-bank/gst" className="hover:text-gold transition-colors">
            GST
          </Link>
          <span>/</span>
