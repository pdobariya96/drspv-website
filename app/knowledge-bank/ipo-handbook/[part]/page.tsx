...
import { TrendingUp, ArrowLeft, ArrowRight, ThumbsUp, ThumbsDown, MessageCircle } from "lucide-react";
...
export async function generateStaticParams() {
  ...
  return articles.map((a) => ({ part: a.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { part } = await params;
  const article = getKnowledgeArticle("ipo-handbook", part);
...
export default async function IPOHandbookPartPage({ params }: PageProps) {
  const { part } = await params;
  const article = getKnowledgeArticle("ipo-handbook", part);
  if (!article) notFound();

  const articles = getKnowledgeArticles("ipo-handbook");
  const currentIndex = articles.findIndex((a) => a.slug === part);
    ...
    <div className="mx-auto max-w-7xl px-4 py-10 lg:grid lg:grid-cols-[260px_1fr] lg:gap-10">
      ...
      <aside className="hidden lg:block">
        <div className="sticky top-24">
          <div className="flex items-center gap-2 mb-4">
            <TrendingUp className="h-4 w-4 text-ipo-amber" />
            <span className="text-xs font-medium text-ipo-amber uppercase tracking-wider">
              IPO Handbook
            </span>
          </div>
          <nav className="space-y-1">
            {articles.map((a) => (
              <Link
                key={a.slug}
                href={`/knowledge-bank/ipo-handbook/${a.slug}`}
      ...
      <article className="min-w-0">
        <div className="flex items-center gap-2 text-xs text-muted mb-6">
          ...
          <span>/</span>
          <Link href="/knowledge-bank/ipo-handbook" className="hover:text-gold transition-colors">
            IPO Handbook
          </Link>
          <span>/</span>
          <span className="text-text">Part {article.frontmatter.chapter}</span>
        </div>
        ...
        <div className="flex items-center gap-4 text-xs text-dim mb-8">
          {article.frontmatter.readTime && <span>{article.frontmatter.readTime} read</span>}
        ...
        <div className="mt-12 grid gap-4 sm:grid-cols-2 border-t border-white/[0.06] pt-8">
          {prev ? (
            <Link
              href={`/knowledge-bank/ipo-handbook/${prev.slug}`}
          ...
          {next && (
            <Link
              href={`/knowledge-bank/ipo-handbook/${next.slug}`}
