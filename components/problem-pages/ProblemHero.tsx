    ...
    <section className="relative overflow-hidden bg-bg">
      ...
      <div className="relative mx-auto max-w-4xl px-4 pb-16 pt-28 text-center sm:pb-24 sm:pt-36">
        ...
        <div className="mt-10 flex flex-col items-center gap-4 sm:flex-row sm:justify-center">
          <a
            href={`tel:+${WA}`}
            className="inline-flex items-center gap-2.5 rounded-lg bg-gold px-7 py-3.5 text-sm font-semibold text-white shadow-lg  transition-all duration-200 hover:brightness-110 hover:scale-[1.02] active:scale-[0.98]"
          >
        ...
        {stats.length > 0 && (
          <div className="mt-12 flex flex-wrap items-center justify-center gap-3">
            {stats.map((stat) => (
              <div
                key={stat}
                className="rounded-full border border-stone-200 bg-card/50 px-4 py-2 text-xs font-medium text-muted"
              >
                {stat}
              </div>
            ))}
